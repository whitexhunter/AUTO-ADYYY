...
def _get_dashboard_embed(discord_id):
    ...
    for s in storage.get_user_subscriptions(discord_id):
        if s["status"] == "confirmed":
            try:
                ...
                if expires.replace(tzinfo=None) > now.replace(tzinfo=None):
                    embed.add_field(
                        name="⏳ Expires",
    ...
    if user and user.get("trial_active"):
        try:
            ...
            if te.replace(tzinfo=None) > now.replace(tzinfo=None):
                embed.add_field(
                    name="🎯 Trial",
...
class ChannelCampaignModal(discord.ui.Modal):
    def __init__(self, did, aid):
        ...
        self.discord_id = did
        self.account_id = aid
        self.add_item(discord.ui.InputText(label="Campaign Name", placeholder="My Campaign", max_length=50, required=True))
        self.add_item(discord.ui.InputText(label="Channel IDs (comma-separated)", placeholder="123456789, 987654321", required=True))
        self.add_item(discord.ui.InputText(label="Messages (separate with ||)", style=discord.InputTextStyle.long, placeholder="Hello! First message with multiple lines...", required=True))
        self.add_item(discord.ui.InputText(label="Image URLs (one per msg, || sep)", placeholder="https://i.imgur.com/img.jpg", style=discord.InputTextStyle.long, required=False))
    ...
    async def callback(self, interaction):
        ...
        embed = discord.Embed(title=f"✅ {name} Running!" if started else f"✅ {name} Created!", color=discord.Color.green())
...
class DmCampaignModal(discord.ui.Modal):
    def __init__(self, did, aid):
        ...
        self.discord_id = did
        self.account_id = aid
        self.add_item(discord.ui.InputText(label="Campaign Name", placeholder="Auto-Reply Campaign", max_length=50, required=True))
        self.add_item(discord.ui.InputText(label="Reply Messages (one per line)", style=discord.InputTextStyle.long, placeholder="Thanks for your message!", required=True))
...
class RedeemKeyModal(discord.ui.Modal):
    ...
    async def callback(self, interaction):
        ...
        plan = result["plan"]
        sid = str(uuid.uuid4())
        exp = "2099-12-31T23:59:59" if plan == "lifetime" else (datetime.now(timezone.utc) + timedelta(days=30)).isoformat()
        storage.add_subscription({
            "id": sid, "discord_id": self.discord_id, "plan": plan,
            "amount": 0, "status": "confirmed",
            "created_at": datetime.now(timezone.utc).isoformat(), "expires_at": exp
        })
